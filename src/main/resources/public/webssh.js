(() => {
  ...
  Vue.createApp({
    ...
    methods: {
      ...
      async disconnect() { if (!this.connection) return; if (this.settings.confirm && !await this.confirmDialog('断开 SSH 连接？', `将断开 ${this.connectionLabel} 的终端和文件会话。`, '断开连接', true)) return; const old = this.connection; await this.closeStartMenu(); try { await this.api(`/api/v1/connections/${encodeURIComponent(old.id)}`, { method: 'DELETE' }); } catch (_) {} this.disconnectTerminal(); Object.values(this.screenWindows).forEach(win => { win.socket?.close(); win.terminal?.dispose(); }); this.screenWindows = {}; this.stopMetrics(); this.addHistory('DISCONNECT', `${old.username}@${old.host}`); this.connection = null; this.screenSessions = []; this.serverTimeOffset = 0; this.persistActive(); this.desktopEntries = []; this.fileEntries = []; this.toast('连接已断开'); },
      async rebootServer() { if (!this.connection) return this.toast('请先连接 SSH 主机', 'error'); if (!await this.confirmDialog('重启远程服务器？', `将立即重启 ${this.connectionLabel}，所有 SSH 和 Screen 连接都会中断。`, '立即重启', true)) return; const old = this.connection; await this.closeStartMenu(); try { await this.api(`/api/v1/connections/${encodeURIComponent(old.id)}/power`, { method: 'POST', data: { action: 'reboot' } }); this.addHistory('REBOOT', `${old.username}@${old.host}`); this.disconnectTerminal(); Object.values(this.screenWindows).forEach(win => { win.socket?.close(); win.terminal?.dispose(); }); this.screenWindows = {}; this.stopMetrics(); this.connection = null; this.screenSessions = []; this.desktopEntries = []; this.fileEntries = []; this.persistActive(); this.toast('服务器重启命令已发送', 'success'); } catch (error) { this.toast(error.message, 'error'); } },
      ...
      dismissMenus(event) { if (!event.target.closest('.context-menu')) this.contextMenu.visible = false; if (!event.target.closest('.start-menu') && !event.target.closest('#brand-button') && !event.target.closest('#dock-launcher')) this.closeStartMenu(); },
      ...
      async toggleStartMenu() { if (this.startMenuOpen) await this.closeStartMenu(); else this.startMenuOpen = true; },
      async closeStartMenu() { if (!this.startMenuOpen) return; const start = document.querySelector('#start-menu'); if (!matchMedia('(prefers-reduced-motion: reduce)').matches && start?.animate) { const animation = start.animate([{ opacity: 1, transform: 'translate3d(0,0,0) scale(1)', filter: 'blur(0)' }, { opacity: 0, transform: 'translate3d(0,12px,0) scale(.92)', filter: 'blur(6px)' }], { duration: 190, easing: 'cubic-bezier(.4,0,1,1)' }); try { await animation.finished; } catch (_) {} } this.startMenuOpen = false; },
      async closeFloatingMenus() { const context = document.querySelector('#context-menu'); let contextAnimation; if (this.contextMenu.visible && context?.animate) contextAnimation = context.animate([{ opacity: 1, transform: 'scale(1)' }, { opacity: 0, transform: 'translateY(5px) scale(.94)' }], { duration: 150, easing: 'ease-in' }).finished; await Promise.allSettled([contextAnimation, this.closeStartMenu()].filter(Boolean)); this.contextMenu.visible = false; },
