...
- 后端：Java 21、Spark Java、Jetty WebSocket、JSch
...
server:
  # Web 服务监听地址
  host: 127.0.0.1
  # 静态页面、REST API 和 WebSocket 共用端口
  port: 8080
ssh:
  # SSH 建连与认证超时（毫秒）
...
网页、REST API 和 WebSocket 使用同一个 `server.port`。浏览器会根据当前页面地址自动建立同源的 `ws://` 或 `wss://` 连接，无需额外开放终端端口。

若需要让局域网中的其他设备访问，可将 `server.host` 设置为 `0.0.0.0`，并在防火墙中放行 `server.port`。对外开放前请先阅读下方的安全说明。
...
确认浏览器能够连接 WebSSH 的 `server.port`。如果通过反向代理部署，需要在同一站点正确转发 `/api/v1/sessions/` 路径的 WebSocket Upgrade 请求，并适当延长连接超时时间。
...
确认 `server.host` 已设置为 `0.0.0.0` 或具体网卡地址，同时放行 `server.port`。修改监听范围会扩大攻击面，请配合防火墙或可信反向代理使用。
